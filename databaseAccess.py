...
def Login(db: Session, email_id: str, password: str):
    influencer = db.query(Influencer).filter(Influencer.email_id == email_id).first()
    if influencer and bcrypt.checkpw(password.encode('utf-8'), influencer.password_hash):
        return {
            "id":influencer.id,
            "name":influencer.name,
            "profile_photo_location":influencer.profile_picture_location
...
def GetInstaPortfolioMetric(db: Session, infleuncer_id: str):
    ...
    url = f"https://graph.instagram.com/v25.0/{id}/insights?metric=engaged_audience_demographics&period=lifetime&timeframe=last_30_days&breakdowns=age&metric_type=total_value&access_token={credentials.access_token}"
    response = requests.get(url)
    data = response.json()
    print(data)
    return response_to_browser
