...
def GetInstaPortfolioMetric(db: Session, infleuncer_id: str):
    ...
    url = f"https://graph.instagram.com/v25.0/{id}/insights?metric=follower_demographics,engaged_audience_demographics&period=lifetime&timeframe=last_90_days&breakdown=age,city,country,gender&metric_type=total_value&access_token={credentials.access_token}"
    response = requests.get(url)
    data = response.json()
    print(data)
    return response_to_browser
