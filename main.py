...
async def connect_social_media(request: Request, platform: str, db: Session = Depends(get_db), token:str = Depends(get_current_user)):
    ...
    if platform == "instagram":
        url = f"https://www.instagram.com/oauth/authorize?force_reauth=true&client_id=1780741403310636&redirect_uri=https://api.influrunner.com/redirect/instagram&response_type=code&scope=instagram_business_basic%2Cinstagram_business_manage_insights"
        return {"url": url}
